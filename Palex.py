...
class main_window(wx.Frame) :
	def __init__(self,parent,id) :
		wx.Frame.__init__(self,parent,id,'PalEX',size=(500,500),style=wx.DEFAULT_FRAME_STYLE^wx.RESIZE_BORDER^wx.MAXIMIZE_BOX)
		...
		testing_menu.Append(test1)
		testing_menu.Append(test2)
		tools_menu.Append(wx.NewId(),'Testing',testing_menu)
		help_topics = wx.MenuItem(help_menu, wx.NewId(), '&Help Topics')
		help_topics.SetBitmap(wx.Bitmap('icons/help_ico.jpg'))
		help_menu.Append(help_topics)
		...
		appname=wx.StaticText(panel,-1,"PalEX",(10,30),(460,-1),wx.ALIGN_CENTER)
...
class about_window(wx.Frame) :
	def __init__(self,parent,id) :
		...
		appname=wx.StaticText(panel1,-1,"Plagiarism Extension Test Version : 1.0",(15,100),(360,-1),wx.ALIGN_CENTER)
		appname.SetFont(font)
		font.SetPointSize(15)
		appname.SetForegroundColour('red')
		appname=wx.StaticText(panel1,-1,"Creator",(150,140),(360,-1),wx.ALIGN_CENTER)
		appname.SetFont(font)
		appname=wx.StaticText(panel1,-1,"---------------------",(150,165),(360,-1),wx.ALIGN_CENTER)
		appname=wx.StaticText(panel1,-1,"Abhishek Gautam",(110,190),(360,-1),wx.ALIGN_CENTER)
		appname.SetFont(font)
...
class training_window(wx.Frame) :
	...
	def __init__(self,parent,id) :
		...
		self.paperNameText=wx.StaticText(self.panel,-1,"paper Name\t : ",pos=(20,200),size=(30,50))
		self.paperNameText.SetFont(font1)
		self.paperNameChoices=wx.Choice(self.panel,-1,pos=(155,200),size=(290,30))
		self.paperNameChoices.SetSelection(0)
		self.paperPrev=wx.TextCtrl(self.panel,-1,"",pos=(50,260),size=(500,200),style=wx.TE_MULTILINE)
		self.paperPrev.SetInsertionPoint(0)
		self.Bind(wx.EVT_CHOICE, self.set_new_author_paper_preview, self.authorNameChoices)
		self.Bind(wx.EVT_CHOICE, self.set_new_paper_preview, self.paperNameChoices)
	...
	def set_new_author_paper_preview(self,event) :
		self.paperNameChoices.SetItems(self.paper_list[self.authorNameChoices.GetSelection()])
		self.paperNameChoices.SetSelection(0)
		file1 = self.authors[self.authorNameChoices.GetSelection()][0]+"/"+self.authors[self.authorNameChoices.GetSelection()][1+self.paperNameChoices.GetSelection()]
		#print file1
		text1 = open(file1,"r").read()
		#print text1
		self.paperPrev.SetValue(text1)	
		self.Refresh()
	...
	def set_new_paper_preview(self,event) :
		file1 = self.authors[self.authorNameChoices.GetSelection()][0]+"/"+self.authors[self.authorNameChoices.GetSelection()][1+self.paperNameChoices.GetSelection()]
		#print file1
		text1 = open(file1,"r").read()
		#print text1
	...
	def add_new_author(self,event) :
		try :
			...
			if len(new_autho)>=3 and len(new_autho[-1])>0 :
				...
				#print new_autho[0::-1]
				self.paper_list.append(new_autho[1:-1])
	...
	class select_new_author_window(wx.Frame) :
		...
		def __init__(self,parent,id) :
			...
			select_papers_button=wx.Button(panel,label="Select papers",pos=(8,70),size=(130,25))
			select_papers_button.SetFont(font1)
			self.Bind(wx.EVT_BUTTON, self.show_select_papers, select_papers_button)
			self.paperText=wx.TextCtrl(panel,-1,"",pos=(150,70),size=(300,-1))#,style=wx.TE_READONLY)
			self.paperText.Bind(wx.EVT_LEFT_DOWN, self.show_select_papers)
			self.paperText.SetInsertionPoint(0)
		...
		def show_select_papers(self,event) :
			wcd = 'Text Files (*.txt)|*.txt'
			open_dlg = wx.FileDialog(self, message='Choose papers', defaultDir=os.getcwd(), defaultFile='',wildcard=wcd, style=wx.FD_OPEN|wx.FD_CHANGE_DIR|wx.FD_MULTIPLE)
			ans=open_dlg.ShowModal()
			#open_dlg.Destroy()
			self.new_author=[]
			papers=""
			papers+=open_dlg.GetDirectory()
			self.new_author.append(open_dlg.GetDirectory())
			for i in range(len(open_dlg.GetFilenames())) :
				papers+=open_dlg.GetFilenames()[i]
				papers+=","
				self.new_author.append(open_dlg.GetFilenames()[i])

			self.paperText.SetValue(papers)
	...
	class features_window(wx.Frame) :
		...
		def __init__(self,parent,id) :
			...
			self.paper_list = copy.copy(paper_list)
			self.features_list = []
			...
			for doc in self.docs :
				...
				j = self.paper_list[i].index(doc.docname)
			...
			self.authorNameChoices.SetSelection(0)
			self.paperNameText=wx.StaticText(self.panel,-1,"paper Name\t : ",pos=(20,80),size=(30,50))
			self.paperNameText.SetFont(font1)
			self.paperNameChoices=wx.Choice(self.panel,-1,pos=(155,80),size=(290,30),choices=self.paper_list[self.authorNameChoices.GetSelection()])
			self.paperNameChoices.SetSelection(0)
			self.paperPrev=wx.TextCtrl(self.panel,-1,self.features_list[0][0],pos=(50,130),size=(500,200),style=wx.TE_MULTILINE)
			self.paperPrev.SetInsertionPoint(0)
			self.Bind(wx.EVT_CHOICE, self.set_new_author_features_preview, self.authorNameChoices)
			self.Bind(wx.EVT_CHOICE, self.set_new_paper_features_preview, self.paperNameChoices)
		...
		def set_new_author_features_preview(self,event) :
			self.paperNameChoices.SetItems(self.paper_list[self.authorNameChoices.GetSelection()])
			self.paperNameChoices.SetSelection(0)
			self.paperPrev.SetValue(self.features_list[self.authorNameChoices.GetSelection()][0])			
			self.Refresh()

		def set_new_paper_features_preview(self,event) :
			self.paperPrev.SetValue(self.features_list[self.authorNameChoices.GetSelection()][self.paperNameChoices.GetSelection()])			
			self.Refresh()
...
class testing_window(wx.Frame) :
	def __init__(self,parent,id) :
		...
		font1.SetPointSize(12)
		self.test1.Disable()
		select_papers_button=wx.Button(panel,label="Select a paper",pos=(8,120),size=(130,25))
		select_papers_button.SetFont(font1)
		self.Bind(wx.EVT_BUTTON, self.show_select_paper, select_papers_button)
		self.paperText=wx.TextCtrl(panel,-1,"",pos=(150,120),size=(300,-1))
		self.paperText.Bind(wx.EVT_LEFT_DOWN, self.show_select_paper)
		self.paperText.SetInsertionPoint(0)
	...
	def start_test_dialog(self,event) :
		if self.test2.GetValue() and self.author2Choices.GetSelection()==self.author1Choices.GetSelection()  :
			...
			answer=box.ShowModal()
			box.Destroy()
		elif len(self.paper1) <2 :
			box=wx.MessageDialog(None,"Please select a paper..!!!!!",'Alert',wx.OK)
			answer=box.ShowModal()
			box.Destroy()
		else :
			box=wx.MessageDialog(None,"Start testing of the paper..!!!",'Alert',wx.YES_NO)
			answer=box.ShowModal()
			box.Destroy()
			if answer==wx.ID_YES :
				self.testing_paper=[]
				self.testing_paper.append(self.test2.GetValue())
				self.testing_paper.append(self.paper1)
				#print "Testing Started with data!!!!","\n",self.testing_paper
				## Place to call the testing Function!!!!!!!
				"""box=wx.MessageDialog(None,"Test Started!!!",'Alert',wx.OK)
				answer=box.ShowModal()
				box.Destroy()"""
				if self.testing_paper[0] :
	...
	def start_binary_testing(self) :
		doc = features(self.testing_paper[1][1],'unknown',self.testing_paper[1][0])
		doc.extract_features()
		...
		print(clfr1.predict(np.array(self.test_data).reshape(1,-1))[0])
	...
	def show_select_paper(self,event) :
		wcd = 'Text Files (*.txt)|*.txt'
		open_dlg = wx.FileDialog(self, message='Choose a paper', defaultDir=os.getcwd(), defaultFile='',wildcard=wcd, style=wx.FD_OPEN|wx.FD_CHANGE_DIR)
		ans=open_dlg.ShowModal()
		#open_dlg.Destroy()
		if open_dlg.GetFilename()!="" :
			...
			papers=""
			papers+=open_dlg.GetDirectory()
			papers+=open_dlg.GetFilename()
			self.paper1.append(open_dlg.GetDirectory())
			self.paper1.append(open_dlg.GetFilename())
			self.paperText.SetValue(papers)
